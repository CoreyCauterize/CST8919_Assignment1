...
def extract_user_identity(user: dict | None) -> tuple[str | None, str | None]:
    if not user:
        return None, None
...
async def callback():
    ...
    try:
        ...
        if isinstance(result, dict):
            user = result.get('user') or result.get('profile') or result.get('claims')
        if not user:
            user = await auth0.get_user(g.store_options)

        user_id, email = extract_user_identity(user)
        log_event('info', 'login_success', user_id=user_id, email=email)
        return redirect(url_for('index'))
    except Exception as e:
        log_event('warning', 'login_failure', error=str(e))
...
async def protected():
    ...
    user_id, email = extract_user_identity(user)
    log_event('info', 'protected_access', user_id=user_id, email=email, route='/protected')
